...
# リポジトリ運用ルール（Windsurf 用ポインタ）

このリポジトリの運用ルール・作業手順の**正本はリポジトリ直下の `AGENTS.md`**（＝各エージェント共通の
メモリ。ops-sync から配布される共通ブロックを含む）。この rule（`trigger: always_on`）は全対話で常時
...
> Windsurf には skill の自動発火機構が無いため、上記のとおり `AGENTS.md` → `docs/` の参照で手順書層を
