...
- `.ops-sync/tasks/` のファイルは同期管理下（手で消しても次回 sync で復活する）。**完了したら
  `種別: task-done` で消化する**（起票と同じ2経路——正本への直接 PR なら ops-sync の
  `tasks/<owner>/<repo>/<ファイル名>` を削除する PR、outbox なら `種別: task-done` の提案。
  選び方は → `docs/outbox-proposal.md`）。作業の「なぜ」は通常どおりこのリポジトリの履歴ファイルに残す。
