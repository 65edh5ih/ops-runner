...
**consumer 側の読み手はまだ無い**（`decision.json` を読んで自動で経路を切り替える実装は入っていない）。
いま `decision.json` が担うのは**人とエージェントが読む推奨**までで、切替は各 consumer の管理画面から
人が行う。この状態で publish だけを先に出しているのは、判断の履歴（いつどちらが推奨されたか）が
無いと切替の是非を後から検証できないため。**「`auto` にすれば自動で切り替わる」と読まないこと**。
