...
- **マージされずクローズされた PR の指摘は在庫に載せない**（表の外の注記に件数だけ出す）。その変更は
  ...
  - **「同じ変更が別 PR で `main` に入るなら、その PR にも同じ指摘が付く」は成り立たない**（＝注記を落として
    完全ドロップにはできない）。実例: nikki-san で放棄された PR（#547）の指摘と同じコードが別 PR（#548）で
    `main` に入ったが、**その PR には Codex のレビューが1本も付かなかった**。欠陥を指す文はクローズ済み PR の
    スレッドにしか無く、それを読んだ人が起こした後続 PR（#551）で直るまで `main` に残っていた。注記が埋める
    のはこの穴で、頻度は低いが代替手段が無い。
