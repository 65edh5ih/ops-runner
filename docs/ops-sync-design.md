...
- **上り（提案）**: consumer → ops-sync。consumer のエージェントは**配布された複製を書き換えず、正本のある
  ...
  **エージェントは自分でマージしない**。正本に入る条件は経路で分かれる:
  ...
  - **outbox 提案** … 提案元が private リポジトリで、かつ前提が変わっていないと機械確認できたものは
    **自動マージ**（→ 下記「上りの認可」）。承認は*提案元でオーナーが提案ファイルを含む PR を main へ
    マージした時点*で済んでおり、取り込み PR の自動マージは同じ変更を二度承認させないための自動化。
    条件を満たさないもの（提案元が public・ベース不一致/欠落・常時層の肥大化など）は
    **オーナーのレビュー待ち**になる。
...
**`shared-file` の鮮度検査と書き込みはどちらもバイト単位で、正規化を挟まない**（`common-block-edit` は
埋め込まれるテキストなので trim した上でハッシュする）。共有実ファイルはバイト一致で配られる以上、
前後の空白も差分だからで、片方だけ正規化すると**鮮度検査を通った提案が提案本文と違うバイトを書く**
（末尾改行だけの変更が検出されず、次の全文置換で黙って巻き戻る）。読みやすさのために trim や
末尾改行の付与を足さない——検査と書き込みが同じバイトを見ていることが、この経路の前提。
本文は**文字列にデコードせず `Buffer` のまま**扱う（utf8 でデコードすると不正な UTF-8 バイトが
U+FFFD に化けて戻らない。frontmatter は自前の書式なので utf8 で読んでよく、境界の位置だけ
バイト単位で求める）。
...
**読み手を実装しているかは consumer ごとに違う**（判断側は読み手の有無を知らない）。実装している
consumer では、その管理 Worker が定期的に `decision.json` を読み、policy が `auto` の対象について
**推奨と実効経路が違えば自分で切り替える**——`auto` のままにすると人手なしで本番の経路が変わる。
実装していない consumer では `decision.json` は**人とエージェントが読む推奨**までで、切替は管理画面から
人が行う。**どちらなのかは自分のリポジトリの実装で確かめる**（MUST。`auto` を選ぶ前に、その対象の
読み手があるかを見る）。読み手が無い側でも publish を先に出しているのは、判断の履歴（いつどちらが
推奨されたか）が無いと切替の是非を後から検証できないため。
...
- **エントリが消える条件は「resolve された」だけ**（古くなったから、ではない。例外は下記の
  「マージされずクローズされた PR」で、そのときは表から消えるかわりに ⚪ の注記に出る＝黙って
  落ちる経路は無い）。直近スキャンは最近更新された
...
- **マージされずクローズされた PR の指摘は在庫に載せない**（表の外の注記に件数だけ出す）。その変更は
  `main` に入っていないので直す対象が無く、載せると消化プロトコル（全件を決着させる）が「理由を返信して
  resolve」という実コストを必ず要求する。放棄された PR は増え続けるため、載せ続けると表が死に行で埋まって
  読まれなくなる——この一覧にとって最悪の壊れ方。ただし**放棄された PR の指摘が `main` に効いていることは
