...
- **読み戻した slice を最新だと決めてかかる**: publish は本体ジョブの後に走り、掃除でブランチを
  作り直すこともある。**「直った」を slice で確認するときは、読んだ内容自身の時刻**（`checked_at`
  等のフィールド、無ければそのコミットの時刻）**が今回の run のものかを見る**。これを見ずに前回の
  内容を読み、「修正が効いていない」と誤報しかけたことがある。
