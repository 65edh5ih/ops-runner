   ...
   - **詳細を実行側のジョブログに出さない**（MUST）。public リポジトリの run ログは world-readable で、
     ...
     **ファイルへリダイレクトする**（`node … > logs/ci/scripts/<name>.log 2>&1`。パイプが無いので
     ...
     3. **`with:` に渡す値そのもの**。`uses:` ステップの入力は必ずログに出る。よって
        ...
        **鍵の無いハッシュを秘匿の手段にしない**（MUST NOT）——ファイル名のような**推測できる入力**は、
        観測者が候補をオフラインでハッシュして突き合わせれば復元できる（12桁＝48ビットの一致確認は
        総当たりを妨げない）。一意性が要るなら**既に公開されている値**（対象リポジトリ名など）か、
        公開側から得られないランダム値・鍵付きハッシュを使う（例: cleanup ブランチ名は
        `ops-sync/cleanup-<consumer 名>`。consumer 名は `consumers.txt` で公開済みなので新しい情報が
        漏れず、同じ相手の掃除 PR を再利用する性質も保てる）。
...
- **外部 API の失敗を `status` だけログに残す**: `status 400` しか残っていないと、**誰も原因に
  辿り着けないまま信号が壊れたまま残り続ける**（実際に数日そうなった）。API が本文に返す
  `code` と `message` まで残す（公開先が world-readable なので**本文全体ではなく code と message
  だけ**。値・トークンは載せない）。「どのパラメータが拒否されたか」はたいていそこにしか無い。
- **読み戻した slice を最新だと決めてかかる**: publish は本体ジョブの**後**に走るので、直後に読むと
  まだ前回の内容が返る。しかも `latest` は同じパスへ**上書き**されるため、古い内容も見た目は「今の値」に
  なる（`ci-logs` は追記型でブランチ自体は作り直されないが、それは鮮度を保証しない）。**「直った」を
  slice で確認するときは、読んだ内容自身の時刻**（`checked_at` 等のフィールド、無ければそのコミットの
  時刻）**が今回の run のものかを見る**。これを見ずに前回の内容を読み、「修正が効いていない」と
  誤報しかけたことがある。
