...
- **`<request_id>`**: `[A-Za-z0-9._-]+`。結果スライスのパスになる。取得ごとに一意にする（衝突回避）。
  **`_invalid-` で始まる ID は使えない**（退避スライス用に予約。→ 手順6）。使うと弾かれて退避先に落ちる。
- **結果ブランチ `net-fetch-results`**（実行したリポジトリ内）。スライスは `net-fetch/<request_id>/`。
  ...
  **このブランチは揮発する**——書き込みのたびに orphan 1コミットへ書き換えられ、**TTL を過ぎた
  ...
  - **読了後の cleanup dispatch**（→ 手順6）: 読み終えたスライスを齢に関係なく即削除する。
    ブランチ上の滞在が TTL ではなく実際の読み取り時間（数分）で終わるのはこの経路のおかげ。
    使ったときだけ 1 run なので、休眠中は 1 分も課金されない。
    **要求の強さはモードで違う**——集約モード（public）は MUST・分散モード（private）は MAY。
    理由と判断は手順6（正本）を見ること。
  ...
  - **分散モード（private）**: 失効は次の取得時にしか走らないので **次に net-fetch を使うまで（上限なし）**。
    非公開なので世界公開にはならないが、「3日で消える」と当てにしないこと。ブランチごと消したいなら
    `git push origin --delete net-fetch-results`。

  ただし上のどれも**「消去」ではない**——TTL 失効も cleanup もブランチ削除も到達不能化にすぎず、
  公開中に SHA を握った相手はその後も取得できる（→ 下の「縛り」節）。上の「約2時間」は
  **発見可能性の上限**であって、開示が2時間で取り消せるという意味ではない。
...
6. **読了後に cleanup を dispatch する**。手順4と同じ workflow を、`request_id`（読んだものと同じ）と
   **`cleanup: 'true'`** を渡して起動する（`url` は不要）。読了済みスライスが齢に関係なく即削除され、
   ...
   - 取得が `rejected`／`error` で終わった場合も、**同じ `request_id` を渡すだけでよい**。
     `request_id` が構造検査（`[A-Za-z0-9._-]+`）や secret 判定に引っかかった取得は、結果が
     `net-fetch/<request_id>/` ではなく **`net-fetch/_invalid-<digest>/`** へ退避されている。
     この digest は生の `request_id` から決まるので、cleanup ジョブが同じものを組み立てて
     **通常スライスと退避スライスの両方**を落とす。退避先を自分で調べて渡す必要は無い。
   - 完了条件: run が **success** で終わる。対象スライスが既に無い場合も「nothing to drop」で success。
     **push に失敗した場合はこの run が赤くなる**ので、緑を**「スライスが結果ブランチから外れた」**の
     証拠として使ってよい（`fail-on-publish-error: 'true'`）。赤で終わったら外れていないので、再 dispatch する。
     **緑は「取得内容が消去された」ではない**——ブランチ先端から辿れなくなるだけで、公開中に SHA を
     記録した相手は依然取得できる（→「縛り」節）。集約モードで機微を取得してしまったときに、
     cleanup の緑を「回収できた」と読まない（MUST NOT）。
...
- **取得結果が git 履歴に堆積しない**: 結果ブランチは毎回 orphan 1コミットに書き換えられる。削除が
  ...
  **cleanup を省いた private の休眠中は最後のスライスが次の取得まで残り、上限が無い**。
  詳細は上の「結果ブランチ」節。
- **削除は「到達不能化」であって消去ではない**（この仕組みが**保証しないこと**。取り違えると設計を誤る）:
  cleanup・TTL 失効・ブランチ削除のいずれも、やっているのは**ブランチ先端から辿れなくすること**だけ。
  到達不能になった commit・blob は**リモートに残り**、ブランチが生きている間に SHA を記録した相手は
  その後も取得できる（GitHub には GC を強制する API が無く、いつ回収されるかは保証されない）。
  したがって縮められるのは**発見可能性と堆積**であって、**開示そのものは取り消せない**。
  **public な集約モードに流した時点で恒久的に開示されたものとして扱う**（MUST）——
  「あとで cleanup するから数分で消える」を機微な取得を public 経路に流す根拠にしない（MUST NOT）。
  機微性の判断はモード選択（上のモード節）で行い、cleanup を安全弁として当てにしない。
- **応答本文がジョブログに残らない**: 本文の出口は結果ブランチ1本だけ。ジョブログには `meta` と
  `bytes`/`sha256` しか出ないので、ブランチ側を消せば**本文はブランチにもログにも無くなる**
  （ログは Artifact and log retention に従って残るが、そこに本文は無い）。
  ただしこれは**「本文が消去された」ではない**——ブランチから外した後も、公開中に SHA を握った相手は
  到達不能な blob として取得できる（上の「削除は到達不能化であって消去ではない」）。
  この項が保証するのは**出口が1つしかない**ことまでで、その1つを塞いでも開示は取り消せない。
