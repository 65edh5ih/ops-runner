...
2. **前提条件を確認する**（MUST）。消す対象が列かテーブルかで見るものが違う。
   ...
   **テーブルを消す場合**: `DROP TABLE` は**依存オブジェクトがあってもその場でエラーにならないことがある**
   ...
   SELECT type, name, sql FROM sqlite_master
    WHERE NOT (type = 'table' AND name = '<table>' COLLATE NOCASE)
      AND sql LIKE '%<table>%';
   ```

   **除外は「対象テーブル自身の行」だけに絞る**（MUST）。ここを `name <> '<table>' COLLATE NOCASE` と
   書くと、**名前が大小違いで一致する別種オブジェクトまで一緒に除外される**。トリガはテーブルと
   別の名前空間なので同名で共存でき（下記「`type = 'table'` を落とさない」と同じ性質）、たとえば
   **他テーブルに付いたトリガ `log` が `Log` へ書いている**場合、それがこの網から消える。しかもそのトリガは
   `DROP TABLE Log` の道連れにならない（道連れになるのは対象テーブル自身に付いたトリガ・索引だけ）ので、
   **事前確認をすり抜けて壊れたまま残る**。`type` と組で除外すれば、対象自身の DDL だけが落ちる。
...
4. **消えたことを読み取り専用で検算する**（MUST）。
   ...
   **テーブルの場合**は、テーブル自身が消えたことに加えて、**手順2で「参照している」と判断した
   ...
   -- (b) 手順2で「参照している」と判断した依存オブジェクトが残っていないか（type と name の組で列挙する）
   SELECT type, name FROM sqlite_master
    WHERE name COLLATE NOCASE IN ('<dep1>', '<dep2>');
   ```

   **`type = 'table'` を落とさない**（MUST）。トリガはテーブルと別の名前空間なので**同名で共存できる**
   （実測: テーブル `Log` と、**別テーブルに付いた**トリガ `log` が並存し、`DROP TABLE Log` 成功後も type 無しの
   数え方では 1 のまま。対象テーブル自身に付いたトリガは道連れに消えるので、残るのは別テーブル側のもの。
   なおビュー・索引はテーブルと**同じ**名前空間で、テーブルと同名では作れない——共存しうるのはトリガだけ）。
   落とすと、この doc 自身が戒めている**満たせないゲート**になる。

   **完了条件**: `table_remains` が 0 で、(b) の結果に**手順2で列挙した `type` と `name` の組が無い**。
   (b) は名前だけで引いているので、**空であることは要求しない**（MUST NOT）——同名で共存できる以上、
   手順2の対象とは別種の無関係なオブジェクトが引っかかりうる（例: 消したビュー `dep` とは別に、無関係な
   トリガ `DEP` が残っている）。空を要求すると、消し忘れが無いのに満たせないゲートになる。
