...
2. `<database_id>` が**本番**の DB であることを確認する（デプロイに使われている設定・環境の
   バインディングと一致するか。名前の似た staging/preview を掴んでいないか）。
   **完了条件**: 本番構成が指す ID と一致している。一致を示せないなら停止して確認する（MUST）。
...
- **ステージング/preview の DB を掴んだまま「0 件だった」と本番の結論にする**。→ 環境別バインディングが
  あるとトップレベル定義は本番とは限らない。手順2で本番構成の ID と一致することを先に示す。
