...
- `<database_id>`: **本番にデプロイされている構成が指す** D1 の ID。設定ファイルの形式はリポジトリに
  よって違い（`wrangler.toml` / `wrangler.json` / `wrangler.jsonc`）、環境別バインディング
  （`[env.<name>.d1_databases]` 等）がある場合は**トップレベルの定義が本番とは限らない**。
  実際にデプロイされている環境の定義から解決する（MUST）。
