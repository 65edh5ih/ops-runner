...
workflow を走らせてよいか**を判断するための手順。あわせて、**private リポジトリの定期実行（cron）の
頻度・時刻を決める**ときの判断軸も置く（→ 下記「定期実行の頻度と時刻の決め方」）。枠は**アカウント単位**で、
`65edh5ih` 配下の private リポジトリ（consumer）はすべて同じ枠を共有する。
...
- **private リポジトリの定期実行（cron）の頻度・時刻を決める／変えるとき**（→ 下記「定期実行の頻度と
