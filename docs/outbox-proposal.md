...
- **エージェントは自分でマージしない**（MUST NOT）。やるのは PR ないし提案を出すところまで。
  全 consumer の振る舞いを一斉に変える変更なので、承認を1点に集約する。
  **どちらの経路でもオーナーのマージを1回通る**——変わるのはその場所だけ:
  - **直接 PR** … オーナーが **ops-sync の PR** をマージする。
  - **outbox 提案** … オーナーが**提案ファイルを載せた consumer 側の PR** をマージする（それで初めて
    提案が `main` に載り、collect の対象になる）。その後の ops-sync 側の取り込み PR は、提案元が
    private で「提案が書かれた後に前提が変わっていない」と機械確認できれば**自動マージ**される
    （条件は下記「取り込みの流れ」）。**同じ変更を二度承認させないための自動化**であって、
    承認を省いているのではない。条件を満たさないものはオーナーのレビュー待ちになる。
...
public 発の outbox で承認が1回増えることだけ）。
