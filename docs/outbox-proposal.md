...
- **エージェントは自分でマージしない**（MUST NOT）。やるのは PR ないし提案を出すところまで。
  全 consumer の振る舞いを一斉に変える変更なので、承認を1点に集約する。正本に入る条件は経路で分かれる:
  - **直接 PR** … ops-sync 側の**人間のレビュー付きマージ**（MUST）。
  - **outbox 提案** … 提案元が private リポジトリで、かつ ops-sync 側が「提案が書かれた後に前提が
    変わっていない」と機械確認できたものは**自動マージ**される（条件は下記「取り込みの流れ」）。
    この経路の承認は**提案を書いたこのセッションでユーザーが見たこと**そのものなので、**ユーザーが
    見ていない提案を outbox に置かない**（MUST NOT）。条件を満たさないものはオーナーのレビュー待ちになる。
...
2. **機械的に安全と確認できた提案は、取り込み PR が自動マージされる**（→ sync workflow が全 consumer へ
   ...
   だけで、提案内容そのものの妥当性は**提案を書いたセッションでユーザーが見た**ことを前提にしている。
